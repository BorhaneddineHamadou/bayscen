...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    """
    # We change the values of these class variables and then use these class variables in other classes of our code
    ...
    def register_actor(actor):
        ...
        if actor in CarlaDataProvider._actor_velocity_map:
            raise KeyError(
                "Vehicle '{}' already registered. Cannot register twice!".format(actor.id))  # Identifier for this actor. Unique during a given episode.
        else:
            CarlaDataProvider._actor_velocity_map[actor] = 0.0  # actor = self.ego_vehicles[i], so it's a dictionary, i.e., velocity of each vehicle w.r.t the vehcile in the dictionary
    ...
    def create_blueprint(model, rolename='scenario', color=None, actor_category="car"):
        ...
        if blueprint.has_attribute('role_name'):
            blueprint.set_attribute('role_name', rolename)  # here we name ego vehicle as hero!ZZZZZZZZZZZZZZZZZZZZZ
    ...
    @staticmethod
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,  # ROLENAME IS HERO FOR EGO VEHICLES ZZZZZZZZZZZZ from scenario_parser
        ...
        """
        blueprint = CarlaDataProvider.create_blueprint(model, rolename, color, actor_category)  # ZZZ Setting rolename of ego to hero on server side here!

        if random_location:
            actor = None
            while not actor:
                spawn_point = CarlaDataProvider._rng.choice(CarlaDataProvider._spawn_points)
                actor = CarlaDataProvider._world.try_spawn_actor(blueprint, spawn_point)  # Return: carla.Actor
            ...
            #print(_spawn_point.location.z)  # -499.29998779296875 da ffffffa!???
            '''
            Hi,
    ...
    def request_new_actors(actor_list):
        ...
        for actor in actor_list:
            ...
            #Z nothing in FutureActor and we're not setting autopilot anyway

            if actor.category == 'misc':
                command.then(PhysicsCommand(FutureActor, True))  # guess FutureActor doesn't matter anyway
