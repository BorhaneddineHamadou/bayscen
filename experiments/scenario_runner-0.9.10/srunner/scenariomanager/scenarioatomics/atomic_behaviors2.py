...
#See definition of py_trees.behaviour.Behaviour here -> https://py-trees.readthedocs.io/en/devel/behaviours.html, to see how all these classes work on ticks
...
class UpdateAllActorControls(AtomicBehavior):
    ...
    def update(self):
        ...
        for actor_id in actor_dict:
            actor_dict[actor_id].run_step()  # which run step been executed. Many implementations of this. Guess depends upon the agent you using! In agent class. Which agent is the other vehicle using in NoSignalJunctionCrossing, lets check later.
...
class KeepVelocity(AtomicBehavior):
    ...
    def __init__(self, actor, target_velocity, other_veh_agentZ, duration=float("inf"), distance=float("inf"), name="KeepVelocity"):
        """
        Setup parameters including acceleration value (via throttle_value)
        and target velocity
        """
        ...
        self._control, self._type = get_actor_control(actor)  # carla vehicle control
        self._map = self._actor.get_world().get_map()
        self._waypoint = self._map.get_waypoint(self._actor.get_location())  # other vehicle loc
        ...
        self.other_veh_agentZ_copy = other_veh_agentZ  # here we make a copy of the other veh automatic control agent with the belief that all the destinations and A* graphs have already been made
        self.high_velocity_value = 5 # 33 #30 #target_velocity #10  #target_velocity  #15  #40
        # has already been set in the InterscetionScenario class before and written in the class variable and would be the same for all object initialized from this class. This concept is very important that I have been using unknowingly for ego vehicle automatic controller xD
        #self.other_veh_agentZ_copy.game_loop_init(carla.Location(x=-85.20, y=-30, z=0.0), False)
    ...
    def initialise(self):
        ...
        self._start_time = GameTime.get_time()
        #self.other_veh_agentZ_copy.agent.set_destination(self._location, carla.Location(x=-85.20, y=-30, z=0.0), clean=True)
        # In case of walkers, we have to extract the current heading
        if self._type == 'walker':
            self._control.speed = self._target_velocity
            self._control.direction = CarlaDataProvider.get_transform(self._actor).get_forward_vector()
    ...
    def update(self):
        ...
        if self._type == 'vehicle':
            if CarlaDataProvider.get_velocity(self._actor) < self._target_velocity:
                ...
                #self.other_veh_agentZ_copy.game_loop_step(self.high_velocity_value)
            else:
                self._control.throttle = 0.0
                #self.other_veh_agentZ_copy.game_loop_step(0.0)
                #self._actor.apply_control(self._control)  # moved to here # no steering command give so I guess the vehicle would stop and not even steer after this
        #self._actor.apply_control(self._control)
        self.other_veh_agentZ_copy.game_loop_step(self.high_velocity_value)

        #print(self.other_veh_agentZ_copy.goal)
...
class SyncArrival(AtomicBehavior):  # class AtomicBehavior(py_trees.behaviour.Behaviour): so it basically is the child of a py tree, so are all the methods inside this class also considered nodes/children that tick with each pytree tick?

#See definition of py_trees.behaviour.Behaviour here -> https://py-trees.readthedocs.io/en/devel/behaviours.html, to see how all these classes work on ticks
# every method inside the pytree behaviour runs on each tick
    ...
    Important parameters:
    - actor: CARLA actor to execute the behavior  # other vehicle
    - actor_reference: Reference actor with which arrival is synchronized  # i.e., the ego vehicle
    ...
    Note: In parallel to this behavior a termination behavior has to be used       # IN PARALLEL
    ...
    def __init__(self, actor, actor_reference, target_location, gain=1, name="SyncArrival"):
        ...
        self._actor_reference = actor_reference  # ego vehicle
    ...
    def update(self):
        ...
        if control_value > 0:
            self._control.throttle = min([control_value, 1])  #0
            self._control.brake = 0
        else:
            self._control.throttle = 0
...
class ChangeNoiseParameters(AtomicBehavior):
    ...
    The behavior terminates after one iteration YAMEEEROOO!!
    """
    ...
    def __init__(self, new_steer_noise, new_throttle_noise,
        ...
        self._noise_to_apply = abs(random.gauss(self._noise_mean, self._noise_std))  # normal distribution!!!!!!!!!!!
...
#So the lane change control commands are executed in it's daddy class WaypointFollower, and as it was mentioned that this class shoudl be run in a 
#parallel tree so I believe that it's daddy class WaypointFollower pytree object  is run in parallel and the plan of waypoints is sent to it from
#this class and the vehicle control commands are executed in the daddy class! 
    """
     This class inherits from the class WaypointFollower.

     This class contains an atomic lane change behavior to a parallel lane.
     The vehicle follows a waypoint plan to the other lane, which is calculated in the initialise method.  # VIP********************
    ...
    A parallel termination behavior has to be used. **Isn't being used :P**
    """
...
class ActorTransformSetter(AtomicBehavior):
    ...
    def __init__(self, actor, transform, physics=True, name="ActorTransformSetter"):
        ...
        #print(self._actor.is_alive)  # it is still true.

    def initialise(self):
        
        #print(self._actor.is_alive)  # does not print anything until ego is moved.
        ...
        if self._actor.is_alive:
            ...
            #print("nice")
            #print(self._actor.is_alive)  #returned true as soon as the ego moved using manual control.py
            #sys.exit("off")
    ...
    def update(self):
        ...
        # WHEN IS MY BEHAVIOUR TICKED? Lets check.
        ...
        if calculate_distance(self._actor.get_location(), self._transform.location) < 1.0:
            ...
            #print("nice")  # doesn't prin't anything before ego is moved
