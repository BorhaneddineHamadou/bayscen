...
class ScenarioRunner(object):
    ...
    def _run_scenarios(self):
        ...
        try:    
            ...
            self.intersection_situations = IntersectionSituations(self._args.Activate_IntersectionScenario_Seed, self._args.IntersectionScenario_Seed, self._args.start_ego, self._args.goal_ego, self._args.start_other, self._args.goal_other, self._args.PathInteraction)  # default value of self._args.use_sit_cov is False!!!!!
        ...
        for config in scenario_configurations:
            ...
            for _ in range(self._args.repetitions):  #  put 2 in this and it repeated from 00! # lets see what happens in reptitions
                ...
                config.weather.cloudiness = self._args.cloudiness # 100
                config.weather.precipitation = self._args.precipitation # 60
                config.weather.precipitation_deposits = self._args.precipitation_deposits # 60
                config.weather.wind_intensity = self._args.wind_intensity # 0
                config.weather.sun_altitude_angle = self._args.sun_altitude_angle
                ...
                config.weather.fog_density = self._args.fog_density # 60
                config.weather.fog_distance = self._args.fog_distance # 0
                config.weather.wetness = self._args.wetness # 60
                config.weather.friction = self._args.friction # 0.1

                print("friction is", config.weather.friction)
                print("cloudiness is", config.weather.cloudiness)
                print("precipitation_deposits is", config.weather.precipitation_deposits)
                #sys.exit("block")
...
class IntersectionSituations:
    ...
    def __init__(self, seed_boolean, seedz, start_ego, goal_ego, start_other, goal_other, PathInteraction):
        ...
        self.other_veh = OtherVehicle()  # Pass ego_veh to other veh so that it can access the ego_veh start and goal locations
        self.start_ego = start_ego
        self.goal_ego = goal_ego
        self.start_other = start_other
        self.goal_other = goal_other
        self.PathInteraction = PathInteraction
    ...
    def start_sit_config_gen(self, approach, activate_env_cond_generation=False):  #Use this method again and again with each repetition of the scenario class
        ...
        # Set ego veh start location
        self.ego_veh.select_ego_start_loc(self.start_ego)
        ...
        self.ego_veh.select_ego_passthroughtrigger_loc(self.start_ego) # Always same as start ego
        # initialize startothertrigger (for ego veh) locs wih this as well
        self.ego_veh.select_ego_startothertrigger_loc(self.start_ego) # Always same as start ego

        # Set ego veh goal loc/endconditiontrigger here
        self.ego_veh.select_ego_endconditiontrigger_loc(self.goal_ego)

        # Set other vehicle start and goal locs. Set other veh triggers. Set the conflictpoint/syncloctrigger also.
        self.other_veh.select_other_vehicle_start_loc(self.start_other)
        self.other_veh.select_other_vehicle_stopothertrigger_loc(self.goal_other)

        self.select_conflictpoint_syncarrival_loc(self.PathInteraction)
...
def main():
    ...
    parser.add_argument('--cloudiness', default=0, type=int,
                            help='Cloudiness environment variable (default: 0)')
    parser.add_argument('--precipitation', default=0, type=int,
                            help='Precipitation environment variable (default: 0)')
    parser.add_argument('--precipitation_deposits', default=0, type=int,
                            help='Precipitation deposits environment variable (default: 0)')
    parser.add_argument('--wind_intensity', default=0, type=int,
                            help='Wind intensity environment variable (default: 0)')
    parser.add_argument('--sun_altitude_angle', default=0, type=int,
                            help='Sun Altitude Angle environment variable (default: 0)')
    parser.add_argument('--fog_density', default=0, type=int,
                            help='Fog density environment variable (default: 0)')
    parser.add_argument('--fog_distance', default=0, type=int,
                            help='Fog Distance environment variable (default: 0)')
    parser.add_argument('--wetness', default=0, type=int,
                            help='Wetness environment variable (default: 0)')
    parser.add_argument('--friction', default=0.0, type=float,
                            help='Friction environment variable (default: 0)')
    parser.add_argument('--start_ego', default='', help='Start position of the ego vehicle')
    parser.add_argument('--start_other', default='', help='Start position of the other vehicle')
    parser.add_argument('--goal_ego', default='', help='Goal position of the ego vehicle')
    parser.add_argument('--goal_other', default='', help='Goal position of the other vehicle')
    parser.add_argument('--PathInteraction', default='', help='PathInteraction between vehicles')
