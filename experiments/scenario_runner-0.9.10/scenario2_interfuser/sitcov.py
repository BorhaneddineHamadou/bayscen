...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        s_num = getattr(self._args, 'scenario_number', 1)
        t_method = getattr(self._args, 'test_method', 'test')
        r_num = getattr(self._args, 'run_number', 3)

        # Create the ScenarioManager with the new arguments
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        if self._args.sync:
            ...
            settings.fixed_delta_seconds = 0.15  # 0.1 seconds for each tick
            self.world.apply_settings(settings)

        # self.traffic_manager.set_synchronous_mode(True) # This is where I changed the synchronous mode
        ...
        if CarlaDataProvider.is_sync_mode():
            print("We are in synchronous mode ...")
            self.world.tick()
        else:
            self.world.wait_for_tick()
    ...
    def extract_object_details(self, obj, depth=0):
        """Recursively extract details of an object."""
        indent = "  " * depth  # Indentation for nested objects
        if isinstance(obj, list):
            return "\n".join(f"{indent}- {self.extract_object_details(item, depth + 1)}" for item in obj)
        elif hasattr(obj, '__dict__'):  # If the object has attributes
            details = "\n".join(f"{indent}{key}: {self.extract_object_details(value, depth + 1)}" 
    ...
    def _run_scenarios(self):
        ...
        try:    
            ...
            self.intersection_situations = IntersectionSituations(self._args.Activate_IntersectionScenario_Seed, self._args.IntersectionScenario_Seed, self._args.test_method)  # default value of self._args.use_sit_cov is False!!!!!
        ...
        for config in scenario_configurations:
            ...
            for _ in range(self._args.repetitions):  #  put 2 in this and it repeated from 00! # lets see what happens in reptitions
                ...
                file_path = 'outputs/config_var_outputs.txt'

                config_details = self.extract_object_details(config)

                # Write to the file
                with open(file_path, 'a') as file:
                    file.write(f"{config_details}\n")
                    file.write("=============================\n\n")
                ...
                # This code is just to save the scenario parameters in json file
                self.weather_states = {
                    ...
                    'sun_altitude_angle': config.weather.sun_altitude_angle, # I added this for my ODD
                    'fog_density': config.weather.fog_density,
                    'fog_distance': config.weather.fog_distance,
                    'wetness': config.weather.wetness,
                    'friction': config.weather.friction,
                }
                t_method = getattr(self._args, 'test_method', 'test')

                file_path = f'outputs/{t_method}/collect_scenarios/weather_states.json'
                self.append_states_to_json(file_path, "weather")
    ...
    def append_states_to_json(self, file_name, states="weather"):
        """
        Append the collected states to an existing JSON file, 
        ...
        try:
            ...
            if states == "weather":
                weather_states = {
                    "weather_states": self.weather_states,  # The actual weather
                }

                # Append the scenario data to the list
                data.append(weather_states)

                # Save the updated data back to the JSON file
                with open(file_name, 'w') as json_file:
                    json.dump(data, json_file, indent=4)

                print(f"{states} states appended to {file_name}")
            

        except Exception as e:
            print(f"Error appending {states} states to JSON: {e}")
...
# SituationGeneration Engine!
    ...
    def __init__(self, seed_boolean, seedz, test_method):  
        ...
        self.ego_veh = EgoVehicle(test_method)
        self.other_veh = OtherVehicle(test_method)
    ...
    def start_sit_config_gen(self, approach, activate_env_cond_generation=False):  #Use this method again and again with each repetition of the scenario class
        ...
        # I added this for my ODD
        self.sun_altitude_angle_key = self.env_conditions.sun_altitude_angle_key
    ...
    def select_conflictpoint_syncarrival_loc(self, conflict_point="c1"):
        ...
        self.conflict_point_for_save = {
            'conflict_point': conflict_point,
        }

        file_path = f'outputs/{self.test_method}/collect_scenarios/conflic_points.json'

        self.append_states_to_json(file_path, "conflic_point")

    def append_states_to_json(self, file_name, states="conflic_point"):
        """
        Append the collected states to an existing JSON file, 
        creating a list of vehicle states over multiple scenarios.
        ...
        try:
            ...
            # Create a structured dictionary for this run
            if states == "conflic_point":
                conflic_point = self.conflict_point_for_save

                # Append the scenario data to the list
                data.append(conflic_point)
    ...
    def generate_environmental_conditions(self, approach, activate=False):
        ...
        if activate:  # if not activated, pass the default values of env conditions paramters
            ...
            if approach == "random":
                ...
                # I added this for my new ODD 
                key_sun_altitude_angle_dict = randint(0, 6)
                self.sun_altitude_angle_key = key_sun_altitude_angle_dict
                self.sun_altitude_angle = self.sun_altitude_angle_dict[key_sun_altitude_angle_dict][0]  # passing the first value of the list which is the parameter value
                self.sun_altitude_angle_dict[key_sun_altitude_angle_dict][1] = self.sun_altitude_angle_dict[key_sun_altitude_angle_dict][1] + 1  # Adding 1 to the counter value which is located at the second place after the parameter in the list at index 1 for this particular key that has been randomly selected
                ...
                # I added this for my new ODD 
                # Setting up a dictionary with the keys and ONLY counter values. Parameters values aren't needed for the sitcov based selection.
                sun_altitude_angle_counts_only_dict = {keys:self.sun_altitude_angle_dict[keys][1] for keys in self.sun_altitude_angle_dict.keys()}
                # Selecting key based on the count values
                key_sun_altitude_angle_dict = self.low_count_to_higher_prob_converter_for_situation_coverage(sun_altitude_angle_counts_only_dict)

                self.sun_altitude_angle_key = key_sun_altitude_angle_dict

                self.sun_altitude_angle = self.sun_altitude_angle_dict[key_sun_altitude_angle_dict][0]  # passing the first value of the list which is the parameter value
                # updating the counter of the selected env condition parameter
                self.sun_altitude_angle_dict[key_sun_altitude_angle_dict][1] = self.sun_altitude_angle_dict[key_sun_altitude_angle_dict][1] + 1
...
class EgoVehicle:
    ...
    def select_ego_start_loc(self, start_location_string="base"):
        ...
        self.ego_start_location = {
            'ego_start_location': start_location_string,
        }
        file_path = f'outputs/{self.test_method}/collect_scenarios/ego_start_locations.json'
        self.append_states_to_json(file_path, "ego_start_location")

    def append_states_to_json(self, file_name, states="ego_start_location"):
        """
        Append the collected states to an existing JSON file, 
        creating a list of vehicle states over multiple scenarios.
        ...
        try:
            ...
            # Create a structured dictionary for this run
            if states == "ego_start_location":
                ego_start_location = self.ego_start_location

                # Append the scenario data to the list
                data.append(ego_start_location)
                ...
                print(f"{states} states appended to {file_name}")
            elif states == "ego_goal_location":
                ego_goal_location = self.ego_goal_location

                # Append the scenario data to the list
                data.append(ego_goal_location)
    ...
    def select_ego_endconditiontrigger_loc(self, ego_goal_location="base"):  # THE GOAL LOCATION OF EGO 
        ...
        self.ego_goal_location = {
            'ego_goal_location': ego_goal_location,
        }
        file_path = f'outputs/{self.test_method}/collect_scenarios/ego_goal_locations.json'
        self.append_states_to_json(file_path, "ego_goal_location")
...
class OtherVehicle:
    ...
    def select_other_vehicle_start_loc(self, start_location_string="base"):
        ...
        self.other_start_location = {
            'other_start_location': start_location_string,
        }

        file_path = f'outputs/{self.test_method}/collect_scenarios/other_start_locations.json'
        self.append_states_to_json(file_path, "other_start_location")

    def append_states_to_json(self, file_name, states="other_start_location"):
        """
        Append the collected states to an existing JSON file, 
        creating a list of vehicle states over multiple scenarios.
        ...
        try:
            ...
            # Create a structured dictionary for this run
            if states == "other_start_location":
                other_start_location = self.other_start_location

                # Append the scenario data to the list
                data.append(other_start_location)
                ...
                print(f"{states} states appended to {file_name}")
            elif states == "other_goal_location":
                other_goal_location = self.other_goal_location

                # Append the scenario data to the list
                data.append(other_goal_location)
    ...
    def select_other_vehicle_stopothertrigger_loc(self, other_vehicle_goal_location="base"):  # THE GOAL LOCATION OF Other vehicle 
        ...
        self.other_goal_location = {
            'other_goal_location': other_vehicle_goal_location,
        }

        file_path = f'outputs/{self.test_method}/collect_scenarios/other_goal_locations.json'
        self.append_states_to_json(file_path, "other_goal_location")
...
def main():
    ...
    parser.add_argument('--test_method', help='Name of the test method (e.g., bayscen)', default='bayscen')
    parser.add_argument('--run_number', help='Run identifier (e.g., run3)', default=3)
    parser.add_argument('--scenario_number', help='Name of the scenario', default=1)
