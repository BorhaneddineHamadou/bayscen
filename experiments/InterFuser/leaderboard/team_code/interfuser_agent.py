...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, vehicle):
        ...
        if self.ensemble:
            for i in range(len(self.config.model)):
                ...
                net.load_state_dict(torch.load(path_to_model_file, map_location=torch.device('cpu'))["state_dict"])
                net.to(torch.device('cpu'))  # Force model to run on CPU # Old : net.cuda()
                net.eval()
            ...
            print('load model: %s' % path_to_model_file)
            self.net.load_state_dict(torch.load(path_to_model_file, map_location=torch.device('cpu'))["state_dict"])
            self.net.to(torch.device('cpu'))  # Force model to run on CPU # Old: self.net.cuda()
            self.net.eval()
    ...
    def tick(self, input_data):
        ...
        # Initialize speed if it's the first time
        # if not hasattr(self, "last_speed"):
        #     self.last_speed = 0.0

        # Update speed if available, otherwise retain the last value
        # if "speed" in input_data:
        #     print("Recieved Speed data:", input_data["speed"])
        #     self.last_speed = input_data["speed"][1]["speed"]
        ...
        # speed = 0.0 # input_data["speed"][1]["speed"]
    ...
    def run_step(self, input_data, timestamp):
        ...
        mes = torch.from_numpy(mes).float().unsqueeze(0).to(torch.device('cpu'))
        ...
        input_data["target_point"] = (
            torch.from_numpy(tick_data["target_point"]).float().to(torch.device('cpu')).view(1, -1)
        )
        input_data["lidar"] = (
            torch.from_numpy(tick_data["lidar"]).float().to(torch.device('cpu')).unsqueeze(0)
        )
        if self.ensemble:
            outputs = []
            with torch.no_grad():
