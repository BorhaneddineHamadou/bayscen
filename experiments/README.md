...
### 3. Execute SitCov and Random Baselines (Ensure `CarlaUE4.exe` is running)

**Note:** Pre-generated scenarios for SitCov and Random baselines are already included in the `generated_scenarios` folder. You can skip this step and follow only **Step 2** to run simulations using the existing Excel files.

However, if you want to regenerate SitCov or Random scenarios from scratch, follow these instructions:
...
2.  Run the script to execute SitCov or Random:
```bash
    python run_sitcov.py --run_number 4
    # OR: python run_random.py --run_number 4 (for Random baseline)
```
    
    This will generate 648 scenarios, corresponding to the same number of scenarios generated by SitCov.

3.  Save the collected scenarios as Excel files (for running them multiple times):
```bash
    python outputs/sitcov/collect_scenarios/save_sitcov_scenarios.py
    # OR: python outputs/random/collect_scenarios/save_random_scenarios.py (for Random)
```

**⚠️ Important:** All these steps have already been done, and the generated scenarios are stored in the `generated_scenarios` folder. You can safely ignore Step 3 and follow **Step 2 (Execute Simulation)** to run experiments using the pre-generated Excel files.
